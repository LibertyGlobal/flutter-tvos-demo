...
- Engine:
  - Use `ios` target to compile `tvos` target
    - It is possible to create a new `tvos` target, but this would impact more files to be changed and therefore more effort to maintain Apple TV support in the engine
...
- Framework
  ...
  - Platform `tvos` not supported, `ios` target cannot be used because without modifications it would the use wrong SDK to compile the application
- Application
...
- Disabling fork commands etc. in &quot;process\_macos.cc&quot;
...
- Install &quot;flutter&quot; version `1.26.0-17.6.pre` ([https://flutter.dev/docs/get-started/install/macos](https://flutter.dev/docs/get-started/install/macos))
...
- **Note:** *Compiling the engine (all targets) requires `29Gb` of free diskspace!*
- Run in /engine/src/ script `sh ninja\_build.sh` (located in script folder of the test application)
